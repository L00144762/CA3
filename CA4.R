...
residual_normalitiy <- function(x){
  hist(x$residuals, main = "histogram of residuals")
  ggqqplot(x$residuals, main = "QQ plot of residuals")
  shapiro.test(x$residuals)
}

residual_normalitiy(x = ulster_lm)
residual_normalitiy(conn_lm)
residual_normalitiy(mnstr_lm)
...
records <- sample(1:nrow(Connaucht), 0.8 * nrow(Connaucht))
# 80/20 split for trainging and testing 
training_data <- Connaucht[records,]
testing_data <- Connaucht[-records,]

linear_model <- lm(`Mean_Rain`~`Mean_Temp`, data = training_data)
...
actual_preds <- data.frame(cbind(actuals = testing_data$Mean_Rain,
...
# alter research questiona nd carry out time series analysis.